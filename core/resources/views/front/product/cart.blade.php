            ...
            <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12">
                @if($cart != null)
                    <ul class="total-item-info">
                        ...
                        <li class="sub-total_all"><span>{{__('Total')}} :</span> {{$be->base_currency_symbol_position == 'left' ? $be->base_currency_symbol : ''}} <span class="cart-total-view">{{number_format($cartTotal,0,',','.')}}</span> {{$be->base_currency_symbol_position == 'right' ? $be->base_currency_symbol : ''}}</li>
                    </ul>
                @endif
                    ...
                    <table class="cart-table">
                        ...
                        <tbody>
                            ...
                            <tr class="remove{{$id}}">
                                ...
                                <td class="price cart_price">{{$be->base_currency_symbol_position == 'left' ? $be->base_currency_symbol : ''}} <span>{{number_format($item['price'],0,',','.')}}</span> {{$be->base_currency_symbol_position == 'right' ? $be->base_currency_symbol : ''}}</td>
                                <td class="sub-total">{{$be->base_currency_symbol_position == 'left' ? $be->base_currency_symbol : ''}} <span>{{number_format($item['qty'] * $item['price'],0,',','.')}} {{$be->base_currency_symbol_position == 'right' ? $be->base_currency_symbol : ''}}</span></td>
                                <td>
                                    <div class="remove">
                                        <div class="checkbox">
            ...
            <div class="row cart-middle">
                <div class="col-lg-6 offset-lg-6 col-sm-12">
                    <div class="update-cart float-right d-inline-block ml-4">
                        <a class="main-btn main-btn-2 proceed-checkout-btn" href="{{route('front.checkout_item')}}"><span>{{__('Checkout')}}</span></a>
                    </div>
